...
describe("App Config", () => {
  context(
    "given env LOG_LEVEL and LOG_TRANSPORTS are not set in the environment variables",
    () => {
      ...
      context("when config is imported", () => {
        ...
        it("should use default values", () => {
          ...
          expect(config.config).to.have.property("port", 3000);
          expect(config.config).to.have.nested.property("cors.origin", true);
          expect(config.error).to.be.undefined;
          expect(config.config).to.have.deep.property("logTransports", [
            "console",
          ]);
        });
      });
